...
public class AList<T> implements ListInterface<T> 
{
   ...
   /*
       public boolean equals(Object other) {
		...
		{
			// the cast is safe within this if statement
			@SuppressWarnings("unchecked")
			AList<T> otherList = (AList<T>)other;
			int otherListLength = otherList.getLength();
			if (numberOfEntries == otherListLength) // must have the same number of objects
			{
				result = true; // assume true until we find a mismatch
				for (int index = 0; (index < numberOfEntries) && result; index++) {
					T thisListItem = list[index];
					T otherListItem = otherList.list[index];
					if (!thisListItem.equals(otherListItem))
						result = false;
				} // end for
			} // end if
		} // end if
